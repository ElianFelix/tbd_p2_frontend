...
export class RequestsComponent implements OnInit {
  ...
  onSubmit() {
    ...
    const account = this.accounts.find(account => account.id == this.requestForm.value.account)
    console.log({...account, balance: (account!.balance! + this.request.amount!)});

    const request$ = this.rServe.updateRequest(this.request);
    const to$ = this.aServe.update({...account, balance: (account!.balance! + this.request.amount!)})

    const fromAccount = this.request.sender
    console.log({ ...fromAccount, balance: (fromAccount!.balance! - this.request.amount!) });

    const from$ = this.aServe.update({ ...fromAccount, balance: (fromAccount!.balance! - this.request.amount!) })
    ...
    forkJoin([request$, from$, to$]).pipe(
      switchMap(([request, from, to]) => {
        const transactionFrom$ = this.tServe.createTransaction(transactionFrom);
